...
/**
 * n8n перезаписує весь рядок крою, тому status і shelf треба слати разом із кількістю —
 * інакше вони затираються, а крій із порожнім статусом випадає з рекомендацій розкрою.
 */
export async function updateCutStockQty(
  dtId: number,
  qty: number,
  status: CutStockItem["status"] = "available",
  shelf = "",
): Promise<void> {
  if (!WEBHOOK_CUTSTOCK) {
    console.info(`[mock] updateCutStockQty dtId=${dtId} qty=${qty} status=${status}`);
    return;
  }
  await ky.post(WEBHOOK_CUTSTOCK, { json: { action: "update", dtId, qty, status, shelf }, timeout: 10000 });
}
